$(function () {
  ...
  test('should set interval from data attribute', 4, function () {
    ...
    ok($('#topPicture').data('bs.carousel').options.interval == 1814)
    $('#topPicture').remove()

    template.appendTo('body').attr('data-modal', 'foobar')
    $('[data-slide]').first().click()
    ok($('#topPicture').data('bs.carousel').options.interval == 1814, 'even if there is an data-modal attribute set')
    $('#topPicture').remove()

    template.appendTo('body')
    $('[data-slide]').first().click()
    $('#topPicture').attr('data-interval', 1860)
    $('[data-slide]').first().click()
    ok($('#topPicture').data('bs.carousel').options.interval == 1814, 'attributes should be read only on initialization')
    $('#topPicture').remove()

    template.attr('data-interval', false)
    template.appendTo('body')
    $('#topPicture').bootstrapCarousel(1)
    ok($('#topPicture').data('bs.carousel').options.interval === false, 'data attribute has higher priority than default options')
    $('#topPicture').remove()
  })
